...
public interface InvoiceDao extends JpaRepository<Invoice, Integer>, JpaSpecificationExecutor<Invoice> {
	...
	// 通过模型分页查询
	@Query(value = "SELECT new com.liwinon.phoneScanning.entity.RecentlyModle(i.invID,i.content,i.pubDate,u.name,i.reimbursement) FROM Invoice i,User u where i.userId=u.userId")
	...
	// 通过模型分页查询我的日志
	@Query(value = "SELECT new com.liwinon.phoneScanning.entity.RecentlyModle(i.invID,i.content,i.pubDate,u.name,i.reimbursement) FROM Invoice i,User u where i.userId=:id AND i.userId=u.userId")
	Page<RecentlyModle> findMyRecently(int id, Pageable pa);

	// 通过日期分页查询我的日志。
	@Query(value = "SELECT new com.liwinon.phoneScanning.entity.RecentlyModle(i.invID,i.content,i.pubDate,u.name,i.reimbursement) FROM "
